# 2017.01.16 18:54:04 中国标准时间
#Embedded file name: F:\YinHuSDK\tools\U8SDKTool-Win-P34\scripts\file_utils.py
import os
import os.path
import re
import platform
...
def getFullOutputPath(appName, channel, appDescript):
    log_utils.info('%s:', appDescript)
    path = getFullPath('output/' + appName + '--' + appDescript.strip() + '/' + channel)
    if not os.path.exists(path):
        os.makedirs(path)
    return path


def getFullOutputOpensPath(appName, channel, appDescript):
    log_utils.info('%s:', appDescript)
    path = getFullPath('output/' + appName + '--' + appDescript.strip() + '/' + channel)
    if not os.path.exists(path):
        os.makedirs(path)
